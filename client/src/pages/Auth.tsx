...
import { Shield, Eye, EyeOff, CheckCircle, AlertCircle, Mail, Lock, ArrowRight, User, Building2 } from 'lucide-react';
...
const LoginView: React.FC<AuthViewProps> = ({ onSwitch }) => {
  const { login } = useAuth();
...
const RegisterView: React.FC<AuthViewProps> = ({ onSwitch }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="relative z-10">
        {isLogin ? <LoginView onSwitch={() => setIsLogin(false)} /> : <RegisterView onSwitch={() => setIsLogin(true)} />}
      </div>
    </div>
  );
};
